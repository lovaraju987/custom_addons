...
class ChannelExtend(models.Model):
    ...
    def _channel_info(self):
        ...
        for channel in self.filtered(lambda x: x.company_id in self.env.companies and x.whatsapp_channel):
            channel_infos_dict.get(channel.id).update({'channel_type': 'WpChannels'})
        return list(channel_infos_dict.values())

    # Move the latest message to the top in the WhatsApp channel sequence.
    def whatsapp_channel_sequence(self, last_interest_dt):
        if self.whatsapp_channel:
            self.channel_member_ids.sudo().write({'last_interest_dt':last_interest_dt})


    # Whatsapp Message Notification in Browser
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        recipients = super()._notify_get_recipients(message, msg_vals, **kwargs)
        message_type = msg_vals .get('message_type',['wa_msgs', 'facebook_msgs', 'insta_msgs']) if msg_vals else message.message_type
        if not recipients and message_type in ['wa_msgs', 'facebook_msgs', 'insta_msgs']:
            recipients_data = []
            if self.is_chat or self.channel_type in ['chat', 'InstaChannels', 'FbChannels']:
    ...
    def _notify_by_web_push_prepare_payload(self, message, msg_vals=False):
        ...
        if self.channel_type in ['InstaChannels','FbChannels']:
            author_id = [msg_vals.get('author_id')] if 'author_id' in msg_vals else message.author_id.ids
            author = self.env["res.partner"].browse(author_id) or self.env["mail.guest"].browse(msg_vals.get("author_guest_id", message.author_guest_id.id))
            payload['title'] = author.name
            payload['options']['icon'] = '/discuss/channel/%d/partner/%d/avatar_128' % (message.res_id, author_id[0])
        return payload

    def whatsapp_channel_search(self, name, limit=20):
        """
        Search WhatsApp Channel from the Discuss
        """
        partners = self.env['res.partner'].sudo().search(['|',('name', 'ilike', name),('mobile','ilike',str(name))], limit=limit).filtered(lambda partner: partner.mapped('channel_provider_line_ids').channel_id)
        search_list = list(partners.mail_partner_format().values())
        return search_list
    ...
    def channel_get(self, partners_to, pin=True):
        ...
        if provider_channel_id and provider_channel_id.provider_id and not partner_info.user_ids:
    ...
    def _set_last_seen_message(self, last_message, allow_older=False):
        ...
        if last_message.isWaMsgsRead == True:
            ...
            if channel_company_line_id.provider_id:
                ...
                if provider_id:
                    message_history = self.env['whatsapp.history'].search([('mail_message_id', '=', last_message.id)])
                    message_id = last_message.wa_message_id if last_message.wa_message_id else message_history.message_id
                    ...
                    if answer.status_code == 200:
                        ...
                        domain = ["&", ("model", "=", "discuss.channel"), ("res_id", "in", self.ids),
                                  ("isWaMsgsRead", "=", False)]
                        if last_message:
                            domain = expression.AND([domain, [('id', '<=', int(last_message))]])
                        messages = self.env['mail.message'].search(domain, order="id DESC")
                        final_messages = messages.filtered(lambda x: last_message.partner_ids.id in x.author_id.ids)
    ...
    def channel_fetched(self):
        ...
        OVERRIDE: To remove the serialization error.
        """
        for channel in self:
...
class ChannelProviderLine(models.Model):
    _inherit = 'channel.provider.line'

    def update_company_in_wa_channel(self):
        wa_channel_provider_lines = self.env['channel.provider.line'].sudo().search([]).filtered(lambda c: c.channel_id and not c.channel_id.company_id and c.provider_id)
        for wa_channel_provider_line in wa_channel_provider_lines:
            wa_channel_provider_line.channel_id.sudo().write({
                'company_id': wa_channel_provider_line.provider_id.company_id.id,
                'provider_id': wa_channel_provider_line.provider_id.id,
            })
